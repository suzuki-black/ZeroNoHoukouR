...
- ★**page2 動的 RAM 実行**(本版で追加。`ramexec_page2_to_ram`/`page2_use_ram`/`page2_use_cart`):
  page1 だけでは**常駐 24KB のうち 0x8000 以降(約4.5KB)が ROM フェッチのまま残り**、そこに毎フレームの
  `stage_update` と SDCC の乗除算ランタイム(`__mulint`/`__divuint`/…)が落ちていた＝**一番熱いコードが
  一番遅い側**。どの関数が 0x8000 を越えるかは `RESIDENT_RELS` の並び順の副作用でしかなく、熱さで
  選ばれていない。→ 常駐 bank2 を別の空きセグメント(seg5)へ複製し、ホット区間だけ page2 も RAM スロットへ。
  出入口は `ramx_use_ram()`/`ramx_use_cart()` の1対に集約(page1 と同じ区間・同じ規律)。
  - page2 の切替コードは page1 に居る＝**自分の足元を切らないので page3 退避 blob が不要**(page1 版との違い)。
  - 複製元は 0x6000-0x7FFF 窓を一時 bank2 に差し替えて読む。したがって**複製関数は 0x4000-0x5FFF に居ること**
    (Makefile がリンク後に `_ramexec_page2_to_ram < 0x6000` を機械検証)。
  - 有効化条件は「複製内容が bank2 と一致」**かつ**「page2 が cart とは別の実 RAM である」の両方
    (チェックサム一致だけでは、切替が効かず書込が捨てられた場合も一致する＝偽陽性。詳細は
    [苦労と教訓.md](苦労と教訓.md))。失敗時は `g_ramx2_ok=0` で **cart のまま=旧挙動**。
  - RAM 中は 0xA000 スワップ窓が消えるため、`data_read`/`bcall` は page1 と同じく区間外でのみ。
  - 実機 A/B 用に `make NO_RAMX2=1` で page2 側だけ無効化したビルドを作れる。
