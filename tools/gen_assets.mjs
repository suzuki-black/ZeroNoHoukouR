...
// ---- 音名で書くための小道具(中ボスの曲から)。n('F#5') → 音階index(0=C2..47=B5) / '-' → 休符 ----
const NOTE_PC = { C:0, 'C#':1, D:2, 'D#':3, E:4, F:5, 'F#':6, G:7, 'G#':8, A:9, 'A#':10, B:11 };
function n(name) {
  if (name === '-') return 255;
  const m = name.match(/^([A-G]#?)(\d)$/);
  const i = NOTE_PC[m[1]] + (+m[2] - 2) * 12;
  if (i < 0 || i > 47) throw new Error(`音域外: ${name}`);
  return i;
}
// 'A5:12 F#5:12 ...' → { mel, mln }
function line(str) {
  const mel = [], mln = [];
  for (const tok of str.trim().split(/\s+/)) { const [nm, len] = tok.split(':'); mel.push(n(nm)); mln.push(+len); }
  return { mel, mln };
}
// ベース: 1小節=4拍(basStep=12)を根音と1オクターブ上で刻む。'D A G ...'(各1小節, 'A/D'=前半A後半D)
function pump(str) {
  const out = [];
  for (const bar of str.trim().split(/\s+/)) {
    const [a, b] = bar.split('/'); const ra = n(a + '2'), rb = n((b ?? a) + '2');
    out.push(ra, ra + 12, rb, rb + 12);
  }
  return out;
}
...
const TRACKS = [
  ...
  (() => {   // 6: 5面 アイオワ(クライマックス, ニ短調・半音階・8分疾走)。1小節=64f(8分=8f)
    //   ★20小節(1280f≒21秒)。元は A の 424f(約7秒)だけでループが短すぎた(実機で指摘)→ 曲調はそのまま後半を足して約3倍。
    //   A(1〜7)   元のメロディそのまま(最後の休符だけ 16→40f で小節に揃えた)
    //   B(8〜13)  B♭→Gm→E♭→A と下属調の側へ。同じ8分の刻みで、最後は C#→G# の半音階で駆け上がる
    //   C(14〜20) 頂点(A5 の連打)→ C→B♭→A→Gm→A と下りてニ短調で締め、先頭の D の連打へ戻る
    //   ★ベースもメロディと同じ 20 小節(元は 4 小節で、メロディとずれながら回っていた)
    const A = {
      ...
      mln: [8,8,8,16,8,8,8,16,8,8,8,16,8,8,8,16,8,8,8,8,8,8,8,16,8,8,8,8,8,8,8,16,16,8,8,16,8,16,32,40],
    };
    const B = line(`A#4:8 A#4:8 F5:8 A#4:8 D5:8 F5:8 A#5:16
                    A5:8 G5:8 F5:8 D5:8 G5:8 A#5:8 A5:8 G5:8
                    D#5:8 D5:8 D#5:8 G5:8 A#5:16 G5:8 D#5:8
                    E5:8 F5:8 E5:8 C#5:8 A4:16 C#5:8 E5:8
                    F5:8 G5:8 A5:8 A#5:8 A5:8 G5:8 F5:8 E5:8
                    C#5:8 D5:8 D#5:8 E5:8 F5:8 F#5:8 G5:8 G#5:8`);
    const C = line(`A5:16 D5:8 F5:8 A5:8 D5:8 F5:8 A5:8
                    G5:16 E5:8 C5:8 E5:8 G5:8 A#5:8 A5:8
                    F5:16 D5:8 A#4:8 D5:8 F5:8 A5:8 A#5:8
                    A5:8 G5:8 F5:8 E5:8 C#5:8 E5:8 A5:16
                    A#5:8 A5:8 G5:8 A#5:8 A5:8 G5:8 F5:8 E5:8
                    F5:8 E5:8 D5:8 C#5:8 D5:8 E5:8 C#5:8 A4:8
                    D5:16 A4:8 F4:8 D4:16 -:16`);
    // ベース: 1小節=8分×8。'D' / 'D/F'(前半D 後半F)。根音と1オクターブ上(元の形 r r r+12 r r r r+12 r)
    const R = { C:0, 'C#':1, D:2, 'D#':3, E:4, F:5, G:7, A:9, 'A#':10 };
    const bas = [];
    for (const bar of 'D A D/F A D/F A D  A# G D# A A#/G A  D C A# A G A D'.split(/\s+/)) {
      const [x, y] = bar.split('/'); const r1 = R[x], r2 = R[y ?? x];
      bas.push(r1, r1, r1 + 12, r1, r2, r2, r2 + 12, r2);
    }
...
function packTrack(t) {
  ...
  if (t.melLoop === undefined && t.drum === 5) {   /* ★中ボスの曲: メロディとベースの尺が一致しないと周回ごとにずれる */
    const fm = t.mln.reduce((x, y) => x + y, 0), fb = t.bas.length * t.basStep;
