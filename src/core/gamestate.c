...
u8  g_fade;       /* 0 以外: 敵をフェードアウト中(残りフレーム)。0 になった瞬間に消す(ovl_power.c) */
u8  g_alert;      /* 1=警報(敵艦発見/敵大将発見)の表示中。縦スクロールと海の塗り直しを止める */
u8  g_mb_n;       /* 中ボスがいま使っているスプライト枚数(末尾から)。エンティティは 32-g_mb_n まで */
u8  g_mb_req;     /* 中ボス: オーバレイが読んでほしい向き(0xFF=なし)。常駐がホット区間の外で読む */
u8  g_jam_t;      /* >0: 5面の中ボス戦でボムを押した(電探妨害の合図を出している残りフレーム) */
u8  g_pwr_icon;   /* 1=パワーアップ段階のアイコンを出す(5面の中ボスの拡大中だけ 0) */
u8  g_mb_front;   /* 1=中ボスを**手前**(HUD の直後)へ置く。2面の中ボスが高度を上げている間だけ */
u8  g_mb_s0, g_mb_end;   /* 常駐が今フレームに中ボスへ与えた枠 [g_mb_s0, g_mb_end)。オーバレイはこの外へ書かない */
u8  g_mb_recol;   /* 1=中ボスの色表が奪われた(メガクラッシュの波)。オーバレイが次のフレームで塗り直す */
u8  g_mb_new;     /* 中ボス: 読み終えた(オーバレイが次のフレームで書く) */
u16 g_mb_bm, g_mb_om;
u16 g_mb_pat_off;   /* 中ボス: パターン表の番地のずらし(4面の2機目=表B) */   /* 中ボス: VRAM に載っている絵の本体/重ねのマス */
