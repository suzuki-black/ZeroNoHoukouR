...
/* ★メガクラッシュ専用: 現在見えている世界行を**全幅で**描き直す(＝画面に描いた稲妻を消す)。
   稲妻は表示リングへ直接描くので、消すにはリングを世界の正本(艦バッファB / 海テンプレ)から
   引き直すのが確実。艦に焼き込んだ炎は B 側にあるので消えない。
   コストは可視14〜15行 × 2KB ≒ 30KB(実測単価で約150ms)だが、クラッシュ中は全部止まっている
   ＝30fps のフレーム予算に縛られないので許容できる(止めてよいと仕様で決まっている)。 */
void scroll_repaint_all(void) {
    s16 r;
