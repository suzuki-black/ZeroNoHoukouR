...
#else
#define HUD_SLOTS 9   /* スコア5桁(0-4)＋残機アイコン(5)＋残機数(6)＋メガクラッシュ(7=棒,8=残数)。
                         ent_draw_all は slot9 以降。★増やすほどゲームのスプライトが減る(最低優先の
                         落ち影/撃破点数から落ちる)ので切り詰めてある。
                         ★パワーアップ段階のアイコンはここに置かない: HUD の枠は敵弾より優先度が高く、
                           1走査線8枚を超えたときに**弾のほうが消えた**(実機で指摘)。entity.c が
                           エンティティの最後＝最低優先で描く(混んだら弾ではなくアイコンが落ちる)。 */
#endif
