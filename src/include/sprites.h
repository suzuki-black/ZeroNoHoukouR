...
#define SPR_WAVE0   144  /* ★津波のコマ4種(144/148/152/156)。16x16 の空き枠はここだけ。
                            ★色は単色(白/淡)。行別カラーテーブルは使えない: 波頭がコマ内で斜めに
                              下がるので、行で色を割ると左端と右端で色の当たる位置がずれ、
                              16px ごとに色が段になって見える(斜め配置の意味が無くなる)。
