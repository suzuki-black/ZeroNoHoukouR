...
enum {
    SFX_NONE = 0,
    /* ★tone B を使う3種は先頭に並べる(sound.c が t <= SFX_LOOP で一括して表から鳴らす) */
    SFX_SHOT,   /* 自機弾: 高→低の下降レーザー "ぴゅん"   */
    SFX_PHIT,   /* 自機被弾: 低い下降の痛み音(tone B)       */
    SFX_LOOP,   /* ★宙返り: 低いエンジン音が引き起こしで上ずっていく "ぶーーーん"(tone B) */
...
void bgm_stop(void);          /* BGM停止＋melody/bass 消音(SFX/noiseは不干渉)。曲/位置は保持する */
