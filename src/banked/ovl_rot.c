...
void mag_shrink(const u8 *src, u8 *dst) {
    ...
    for (r = 0; r < 8; r++) {
        ...
        u8 d = 0, m = 0x80;
        /* ★(0x80 >> i) と書くと sdcc(4.6, ovl12 の --opt-code-size)がシフト回数を 0 に固定した機械語を出し、全部の点が左端の列に
             集まった(5面の中ボスで宙返りの絵が縦線に化けた=実機で指摘)。1 ビットずつずらす印を使う */
        for (i = 0; i < 8; i++, m >>= 1) { if (s & 0xC000) d |= m; s <<= 2; }
        dst[r] = d;
    }
