/* ovl_sink.c — 戦艦の撃沈シーン(ROADMAP P2-B / B1)。RAM オーバレイ OVL7_BANK で動く。
   ...
   ★見本は 1943(カプコン 1987)。見下ろし視点の沈没は「**船尾から水に沈み、見えている艦が船首へ向かって
     短くなって消える**」。艦は動かず、画面は前へ進み続ける。
   ★台本(t は撃沈開始からのフレーム):
...
/* 世界 Y の行 [wy, wy+n) がすべて表示リングに今あるか(リングは 256 行周期。範囲外に書くと 256 上の行を壊す)。
   ★下限は cam ではなく 16 行単位に切り下げた値。スクロールは 16 行ずつリングへ描くので、画面の上端より少し上の行も
     もうリングに描いてある。そこを書き漏らすと、水面が画面の上へ抜けた行に艦の絵が残り、あとで画面に降りてくる(実際に出た)。 */
static u8 in_ring(u16 wy, u8 n) { u16 b = (u16)(cam & 0xFFF0); return (u8)(wy >= b && (u16)(wy + n) <= (u16)(b + 256)); }
...
u8 ovl_sink_frame(void) {
    ...
    /* 画面はゆっくり前へ(1943 と同じく、艦は止まったまま下へ流れていく)。静まってからは止める */
