...
/* ★リングは「帯の入口で外向きにずらし、出口で基準へ戻す」の **2枚組**。
   ...
   ★★帯の幅は見た目の都合ではなく **分割が発火する下限**で決まる。
     ...
     ★2026-09-17 実機turboR実測(検証ROM `scene_hstest.c` の FROZEN WAVE で帯数を変えて確認):
       **間隔 29 行(帯6本)までは全部の帯が効く。22 行(8本)で怪しくなり、17 行(10本)では 2 本しか効かない。**
       openMSX は 14 行でも全部効くので、**エミュレータだけを見て間隔を決めてはいけない**。
       ここを 12→26 行へ広げた(帯幅も 16→28 行)。従来値では実機で「帯の出口」が発火せず、
       うねりが下まで残っていた可能性がある。 */
#define SHOCK_HALF       14    /* 帯の半幅 px(帯幅=28 行) */
#define SHOCK_MIN_GAP    26    /* 直前の分割からこの行数より近い分割は捨てる(発火しないため。実機で 29 行までが安全) */
